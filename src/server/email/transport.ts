...
import { sendTransactionalEmail as sendViaResend, type SendEmailParams, type SendEmailResult } from './resend';
 ...
 */
export async function sendTransactionalEmail(params: SendEmailParams): Promise<SendEmailResult & { provider: EmailProvider }> {
  ...
  if (provider === 'lovable_email') {
    ...
    console.warn('[email] EMAIL_PROVIDER=lovable_email ainda não implementado, usando Resend como fallback', {
      to: params.to,
