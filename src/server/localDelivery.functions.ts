...
export const listLocalDeliveryZones = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: zones, error } = await supabase
      ...
      .select('id, district, name, display_name, parent_zone_id, is_alias, inherits_parent_price, shipping_price, estimated_delivery_time, is_active, notes, sort_order, created_at, updated_at')
...
export const updateLocalDeliveryZone = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const { id, ...patch } = data;
    const { error } = await supabase.from('local_delivery_zones').update(patch as never).eq('id', id);
