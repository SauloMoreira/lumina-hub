...
function extractCosmosImages(data: unknown): string[] {
  ...
  for (const item of arr) {
    if (typeof item === "string" && /^https?:\/\//i.test(item)) out.push(item);
      ...
      const url =
        typeof obj.url === "string" ? obj.url : typeof obj.image === "string" ? obj.image : null;
          ...
          function: {
            ...
            parameters: {
              ...
              properties: {
                ...
                category_hint: {
                  type: "string",
                  description: 'Categoria sugerida em português, p. ex. "Lâmpadas LED"',
                },
                description: {
                  type: "string",
                  description: "Descrição comercial concisa (3-5 frases)",
                },
                tags: {
                  type: "array",
                  items: { type: "string" },
  ...
  .handler(async ({ data }): Promise<BarcodeLookupResult> => {
    ...
    if (!cosmosData) {
      try {
        ...
        const hasData =
          (ai.name && ai.name.trim().length > 0) || (ai.brand && ai.brand.trim().length > 0);
        if (!hasData) {
          return {
            ok: false,
            source: "none",
    ...
    const brand =
      ((cd.brand as Record<string, unknown> | null)?.name as string | undefined) ?? null;
    const category =
