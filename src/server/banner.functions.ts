...
export const generateBannerImage = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const themeHint =
      type === 'free_shipping' ? 'Caminhão de entrega em movimento, ícones de logística, gradiente verde-azulado vibrante.'
      : type === 'category' ? 'Composição com produtos elétricos modernos (disjuntores, cabos, ferramentas) elegantemente dispostos.'
      : type === 'launch' ? 'Visual de lançamento moderno, brilho difuso, partículas de luz, paleta sofisticada.'
      : type === 'seasonal' ? 'Elementos sazonais sutis, paleta temática, ambiente acolhedor e comercial.'
      : type === 'institutional' ? 'Visual corporativo limpo, com ícones de confiança, garantia e atendimento.'
    ...
    if (!url || !url.startsWith('data:image/')) {
      console.error('[generateBannerImage] resposta sem imagem');
