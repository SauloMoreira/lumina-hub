...
export const CART_STATUSES: { value: CartStatus; label: string; color: string }[] = [
  ...
  { value: 'contato_enviado', label: 'Contato enviado', color: 'bg-amber-500/15 text-amber-700 border-amber-500/30' },
  { value: 'recuperado', label: 'Recuperado', color: 'bg-emerald-500/15 text-emerald-700 border-emerald-500/30' },
...
export const detectAbandonedCarts = createServerFn({ method: 'POST' })
  ...
  .inputValidator((d) => z.object({ minutes: z.number().int().min(5).max(60 * 24).default(60) }).parse(d))
  .handler(async ({ data }) => {
...
export const listAbandonedCarts = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const profilesMap = new Map<string, { name: string | null; email: string | null; phone: string | null }>();
    if (userIds.length > 0) {
      const { data: profs } = await supabaseAdmin
        .from('profiles')
        .select('id, name, email, phone')
        .in('id', userIds);
      (profs ?? []).forEach((p) => profilesMap.set(p.id, { name: p.name, email: p.email, phone: p.phone }));
    }
    const leadsMap = new Map<string, { name: string; email: string | null; phone: string | null }>();
    if (leadIds.length > 0) {
      const { data: ls } = await supabaseAdmin
        .from('leads')
        .select('id, name, email, phone')
        .in('id', leadIds);
      (ls ?? []).forEach((l) => leadsMap.set(l.id, { name: l.name, email: l.email, phone: l.phone }));
    }
...
export const getAbandonedCart = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    let profile = null as { name: string | null; email: string | null; phone: string | null } | null;
    if (cart.user_id) {
    ...
    let lead = null as { id: string; name: string; email: string | null; phone: string | null; status: string | null } | null;
    if (cart.lead_id) {
  ...
  .inputValidator((d) =>
    z
      .object({
        id: z.string().uuid(),
        status: z
          .enum(['novo', 'contato_enviado', 'recuperado', 'perdido', 'ignorado'])
          .optional(),
