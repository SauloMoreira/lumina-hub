...
export function CookieBanner() {
  ...
  const handleClose = (action: () => void) => {
    // Feedback visual imediato: inicia animação de saída agora
    setClosing(true);
    // Persiste o consentimento sem bloquear a animação
    startTransition(() => {
      // Pequeno atraso para a animação ser perceptível antes do unmount
  ...
  return (
    <>
      <div
        className="fixed inset-0 bg-black/20 z-[60] pointer-events-none transition-opacity duration-200"
        aria-hidden
        style={{
      ...
      <div
        ...
        className="fixed bottom-0 left-0 right-0 z-[70] bg-card border-t border-border shadow-2xl transition-transform duration-200"
        style={{
      ...
      >
        <div className="container mx-auto px-4 py-5 grid lg:grid-cols-[1fr_auto] gap-5 items-center">
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col gap-2 lg:min-w-[200px]">
            ...
            <Button
              onClick={() => handleClose(rejectOptional)}
              variant="outline"
              size="sm"
              className="w-full"
              disabled={closing}
