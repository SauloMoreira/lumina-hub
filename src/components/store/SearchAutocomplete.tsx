          ...
          {suggestions.map((s, i) => {
            ...
            return (
              ...
              >
                {s.kind === 'product' ? (
                  s.image ? (
                    <img src={s.image} alt="" className="w-10 h-10 object-cover rounded border border-border shrink-0" loading="lazy" />
                  ) : (
                    <div className="w-10 h-10 rounded bg-muted shrink-0 flex items-center justify-center text-muted-foreground"><Box className="w-4 h-4" /></div>
                  )
                ) : (
                  <div className="w-10 h-10 rounded bg-primary-tint text-primary shrink-0 flex items-center justify-center"><FolderTree className="w-4 h-4" /></div>
                )}
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-foreground truncate">{s.name}</p>
                  <p className="text-xs text-muted-foreground truncate">
                    {s.kind === 'product'
                      ? (s.brand ? `${s.brand} · ` : '') + (finalPrice != null ? formatBRL(finalPrice) : '')
                      : 'Categoria'}
                  </p>
                </div>
              </button>
