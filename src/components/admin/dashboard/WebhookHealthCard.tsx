...
export function WebhookHealthCard({ stats }: { stats: DashboardData["webhookStats"] }) {
  ...
  const items = [
    ...
    },
    {
      ...
      tone: stats.errors > 0 ? "text-destructive" : "text-muted-foreground",
    },
  ];
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        {items.map((it) => (
          <div
            key={it.label}
            className="flex flex-col items-start gap-1 p-3 bg-muted/40 rounded-lg"
          >
