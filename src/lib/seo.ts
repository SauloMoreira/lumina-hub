 ...
 */
export function buildSeo(input: SeoInput = {}): { meta: Meta[]; links: { rel: string; href: string }[] } {
  ...
  if (type === 'product' && input.product) {
    ...
    if (input.product.sku) meta.push({ property: 'product:retailer_item_id', content: input.product.sku });
  }
