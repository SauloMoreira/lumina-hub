...
import { getFinanceSettings, updateFinanceSettings, type FinanceSettings } from '@/server/finance.functions';

export const Route = createFileRoute('/admin/financeiro/configuracoes')({
  head: () => buildSeo({ title: 'Configurações financeiras', url: '/admin/financeiro/configuracoes', noindex: true }),
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-6">
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input type="number" step="0.1" value={s.default_min_margin_percent}
                  onChange={(e) => update('default_min_margin_percent', Number(e.target.value))} />
              </div>
              <div>
                <Label>Margem crítica abaixo de (%)</Label>
                <Input type="number" step="0.1" value={s.critical_margin_threshold_percent}
                  onChange={(e) => update('critical_margin_threshold_percent', Number(e.target.value))} />
              </div>
            </div>
            <div className="space-y-3">
              {([
                ['critical_margin_alert_enabled', 'Mostrar alerta de margem crítica no Painel do Dia'],
              ...
              ] as Array<[keyof FinanceSettings, string]>).map(([k, lbl]) => (
                <div key={k} className="flex items-center justify-between gap-4">
                  <Label htmlFor={k}>{lbl}</Label>
                  <Switch id={k} checked={s[k] as boolean} onCheckedChange={(v) => update(k, v as never)} />
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="space-y-4 pt-4 border-t border-border">
            ...
            <p className="text-xs text-muted-foreground">
              Usadas para estimar o líquido. Quando o webhook do Mercado Pago retornar a taxa real, ela tem prioridade.
            </p>
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Pix — % por venda</Label><Input type="number" step="0.01" value={s.mp_fee_pix_percent} onChange={(e) => update('mp_fee_pix_percent', Number(e.target.value))} /></div>
              <div><Label>Pix — taxa fixa (R$)</Label><Input type="number" step="0.01" value={s.mp_fee_pix_fixed} onChange={(e) => update('mp_fee_pix_fixed', Number(e.target.value))} /></div>
              <div><Label>Cartão — % por venda</Label><Input type="number" step="0.01" value={s.mp_fee_credit_percent} onChange={(e) => update('mp_fee_credit_percent', Number(e.target.value))} /></div>
              <div><Label>Cartão — taxa fixa (R$)</Label><Input type="number" step="0.01" value={s.mp_fee_credit_fixed} onChange={(e) => update('mp_fee_credit_fixed', Number(e.target.value))} /></div>
              <div><Label>Boleto — % por venda</Label><Input type="number" step="0.01" value={s.mp_fee_boleto_percent} onChange={(e) => update('mp_fee_boleto_percent', Number(e.target.value))} /></div>
              <div><Label>Boleto — taxa fixa (R$)</Label><Input type="number" step="0.01" value={s.mp_fee_boleto_fixed} onChange={(e) => update('mp_fee_boleto_fixed', Number(e.target.value))} /></div>
              <div className="col-span-2"><Label>Padrão — % por venda</Label><Input type="number" step="0.01" value={s.mp_fee_default_percent} onChange={(e) => update('mp_fee_default_percent', Number(e.target.value))} /></div>
            </div>
          </section>
