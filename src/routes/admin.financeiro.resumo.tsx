...
export const Route = createFileRoute("/admin/financeiro/resumo")({
  head: () =>
    buildSeo({ title: "Resumo financeiro", url: "/admin/financeiro/resumo", noindex: true }),
    ...
    ? [
        {
          ...
          hint: `${data.ordersPaid} pedido(s) pago(s)`,
        },
        {
          label: "Faturamento líquido estimado",
          value: brl(data.estimatedNetRevenue),
          hint: "Após taxa MP estimada",
        },
        ...
        {
          label: "Custo estimado dos produtos",
          value: brl(data.estimatedCogs),
          hint:
            data.itemsWithoutCost > 0 ? `${data.itemsWithoutCost} item(ns) sem custo` : undefined,
        },
        {
          label: "Margem bruta estimada",
          value: brl(data.estimatedGrossMargin),
          hint: `${data.estimatedMarginPercent.toFixed(1)}%`,
        },
        {
          label: "Lucro bruto estimado",
          value: brl(data.estimatedGrossProfit),
          hint: "Margem − taxa MP",
        },
      ]
    : [];
    ...
    <AdminLayout title="Resumo financeiro">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-muted/40 border border-border rounded-lg p-3 mb-6 flex items-start gap-2 text-xs text-muted-foreground">
          ...
          <span>
            Os valores de margem e lucro são estimativas baseadas no custo cadastrado dos produtos e
            nas taxas configuradas em Configurações financeiras.
          </span>
        </div>
