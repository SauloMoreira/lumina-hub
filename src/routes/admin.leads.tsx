    ...
    <AdminLayout title="Leads">
      <div className="bg-card border border-border rounded-xl">
        <div className="p-4 border-b border-border">
          <select value={filterStatus || 'all'} onChange={(e) => setFilterStatus(e.target.value === 'all' ? '' : e.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm">
          ...
          <tbody>
            ...
            {leads.map((l) => (
              <tr key={l.id} className="border-t border-border hover:bg-muted/20">
                ...
                <td className="px-4 py-3"><span className="text-xs px-2 py-0.5 rounded bg-muted">{leadStatusLabel(l.status)}</span></td>
