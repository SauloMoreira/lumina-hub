...
import { Mail, Phone, MapPin, Clock, MessageCircle, Send, Loader2, CheckCircle2 } from 'lucide-react';
...
export const Route = createFileRoute('/contato')({
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'Entre em contato com nossa equipe. Responderemos sua mensagem o mais breve possível.' },
      { property: 'og:title', content: 'Contato — Fale Conosco' },
      { property: 'og:description', content: 'Canais de atendimento, telefone, WhatsApp, e-mail e formulário de contato.' },
    ],
  }),
...
function ContatoPage() {
  ...
  const submit = useMutation({
    mutationFn: () => submitContactMessage({ data: {
      name: form.name.trim(),
    ...
    onError: () => toast.error('Não foi possível enviar agora. Tente novamente em alguns instantes.'),
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!accept) { toast.error('É necessário concordar com o uso dos dados.'); return; }
    ...
    <StoreLayout>
      <div className="container mx-auto px-4 py-12 lg:py-16">
        <div className="max-w-3xl mx-auto text-center mb-10">
          <h1 className="font-display text-3xl lg:text-4xl font-bold tracking-tight">Fale Conosco</h1>
          <p className="mt-3 text-muted-foreground">
            Tire dúvidas, envie sugestões ou solicite atendimento. Retornaremos o mais breve possível.
          </p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-3">
            {company?.support_email && (
              <InfoCard icon={<Mail className="w-5 h-5" />} label="E-mail">
                <a href={`mailto:${company.support_email}`} className="text-primary hover:underline break-all">
                  {company.support_email}
                </a>
              </InfoCard>
            )}
            {company?.support_phone && (
              <InfoCard icon={<Phone className="w-5 h-5" />} label="Telefone">
                <a href={`tel:${onlyDigits(company.support_phone)}`} className="text-primary hover:underline">
                  {company.support_phone}
                </a>
              </InfoCard>
            )}
            ...
            <CardContent className="p-6">
              {done ? (
                <div className="py-12 text-center">
                  ...
                  <Button variant="outline" onClick={() => setDone(false)}>Enviar outra mensagem</Button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Input id="name" required maxLength={200} value={form.name}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input id="phone" maxLength={40} value={form.phone}
                        onChange={(e) => setForm({ ...form, phone: e.target.value })} />
                    </div>
                    <div className="space-y-1.5">
                      <Label htmlFor="subject">Assunto</Label>
                      <Input id="subject" maxLength={200} value={form.subject}
                        onChange={(e) => setForm({ ...form, subject: e.target.value })} />
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="message">Mensagem *</Label>
                    <Textarea id="message" required maxLength={5000} rows={6} value={form.message}
                      onChange={(e) => setForm({ ...form, message: e.target.value })} />
                  </div>
                  <label className="flex items-start gap-2 text-xs text-muted-foreground">
                    <input type="checkbox" className="mt-0.5" checked={accept}
