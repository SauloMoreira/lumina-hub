...
import {
  ShoppingCart, Search, RefreshCw, MessageSquareText, Copy,
  ExternalLink, Building2, Phone, Mail, Sparkles, AlertTriangle,
} from 'lucide-react';
...
import {
  detectAbandonedCarts, listAbandonedCarts, getAbandonedCart,
  updateAbandonedCart, logContactAttempt, CART_STATUSES, type CartStatus,
  ...
  return (
    ...
    >
            ...
            return (
              <button
                ...
                className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                  active ? s.color : 'bg-background text-muted-foreground border-border hover:bg-muted'
                }`}
              >
                {s.label}
              </button>
                  ...
                  <tr key={r.id} className="border-t border-border hover:bg-muted/20">
                    <td className="px-4 py-3">
                      <div className="font-medium flex items-center gap-1.5">
                        {r.customer_name ?? <span className="text-muted-foreground italic">Visitante</span>}
                    ...
                    <td className="px-4 py-3 font-medium">
                      <span className={isHighValue ? 'text-emerald-600' : ''}>{brl(r.subtotal_amount)}</span>
                      {isHighValue && (
                        <Badge variant="outline" className="ml-1 border-emerald-500/40 text-emerald-700 text-[9px]">
                          Alto valor
                        </Badge>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <Button size="sm" variant="ghost" className="h-8 text-xs" onClick={() => setOpenId(r.id)}>
                          Ver
                        </Button>
                      </div>
                    </td>
      ...
      <div className="mt-4 rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground flex gap-2">
        ...
        <div>
          Estes clientes colocaram produtos no carrinho mas não finalizaram. Vale chamar pelo WhatsApp,
          principalmente os de <strong>alto valor</strong>. As mensagens são <strong>manuais</strong> —
      ...
      {openId && (
        <CartDetailDialog
          ...
          onWhatsapp={(id) => { setOpenId(null); setWaCartId(id); }}
        />
      )}
      {waCartId && (
        <WhatsappDialog id={waCartId} onClose={() => setWaCartId(null)} />
      )}
    </AdminLayout>
  );
}
...
// Detalhe
// =====================================================================
function CartDetailDialog({ id, onClose, onWhatsapp }: { id: string; onClose: () => void; onWhatsapp: (id: string) => void }) {
  const qc = useQueryClient();
  ...
  useEffect(() => { if (data?.cart?.notes) setNotes(data.cart.notes); }, [data]);
          ...
          <div className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-3 text-sm">
              <div className="rounded-md border border-border p-3">
                ...
                <div className="font-medium">{data.profile?.name ?? data.lead?.name ?? cart.customer_name ?? '—'}</div>
              ...
              <div className="rounded-md border border-border p-3">
                ...
                <div>Subtotal: <strong>{brl(cart.subtotal_amount)}</strong></div>
                <div className="text-xs text-muted-foreground">
              ...
              <div className="rounded-md border border-border divide-y divide-border">
                ...
                {items.map((it, i) => (
                  <div key={i} className="p-3 flex items-center gap-3">
                    {it.product_image ? (
                      <img src={String(it.product_image)} alt="" className="w-12 h-12 rounded object-cover bg-muted" />
                    ) : (
                      <div className="w-12 h-12 rounded bg-muted" />
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium truncate">{String(it.product_name ?? '—')}</div>
                      <div className="text-xs text-muted-foreground">
            ...
            <div>
              ...
              <Textarea
                ...
                onBlur={() => { if (notes !== (cart.notes ?? '')) updMut.mutate({ notes }); }}
...
function WhatsappDialog({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  const { data } = useQuery({ queryKey: ['abandoned-cart', id], queryFn: () => get({ data: { id } }) });
  ...
  const onOpen = async () => {
    if (!waUrl) { toast.error('Sem telefone válido'); return; }
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        ...
        <DialogFooter className="gap-2">
          <Button variant="outline" onClick={onCopy}><Copy className="w-4 h-4 mr-1" /> Copiar</Button>
