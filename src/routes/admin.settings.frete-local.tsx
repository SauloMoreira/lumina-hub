...
function FreteLocalAdmin() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "active" | "inactive" | "no-price">(
    "all",
  );
  ...
  const stats = useMemo(() => {
    ...
    const prices = zones
      .filter((z) => z.shipping_price !== null)
      .map((z) => Number(z.shipping_price));
    return {
      total: zones.length,
    ...
    <AdminLayout title="Frete Local Maricá/RJ">
                      ...
                      <td className="px-3 py-3 hidden lg:table-cell">
                        <div className="flex flex-wrap gap-1">
                          {zAliases.map((a) => (
                            <span
                              key={a.id}
                              className="inline-flex items-center gap-1 text-xs bg-surface border border-border rounded-full px-2 py-0.5"
                            >
      ...
      {editing && (
        <EditZoneDialog
          ...
          onSaved={() => qc.invalidateQueries({ queryKey: ["admin-local-zones"] })}
        />
      )}
...
function StatCard({
  ...
  return (
    <div
      className={`bg-card border rounded-xl p-3 sm:p-4 ${highlight ? "border-success/40" : warning ? "border-warning/40" : "border-border"}`}
    >
      <div className="text-xs text-muted-foreground uppercase tracking-wide">{label}</div>
      <div
        className={`mt-1 font-display font-bold text-xl ${highlight ? "text-success" : warning ? "text-warning" : ""}`}
      >
        {value}
      </div>
    </div>
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{zone.display_name}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Deixe vazio para "não configurado".
            </p>
          </div>
          <div>
...
function CreateZoneDialog({
  ...
  async function save() {
    if (!name.trim() || !displayName.trim()) {
      toast.error("Preencha nome e nome exibido");
      return;
    }
    setSaving(true);
    const parsed = price.trim() === "" ? null : Number(price.replace(",", "."));
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Nova localidade</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Ex.: Barroco - Itaipuaçu"
              />
            </div>
            <div>
              ...
              <Select value={district} onValueChange={setDistrict}>
                ...
                <SelectContent>
                  {[
                    ...
                    "Itaipuaçu",
                    ...districts.filter(
                      (d) => !["Sede", "Ponta Negra", "Inoã", "Itaipuaçu"].includes(d),
                    ),
                  ].map((d) => (
                    <SelectItem key={d} value={d}>
                      {d}
          ...
          {parentId !== "none" && (
            <label className="flex items-center gap-2 text-sm">
              <Switch checked={inherits} onCheckedChange={setInherits} /> Herdar valor da localidade
              principal
            </label>
          )}
          {!inherits && (
...
function CreateAliasDialog({
  ...
  async function save() {
    ...
    setSaving(true);
    const r = await createLocalDeliveryAlias({
      data: { zone_id: zone.id, alias_name: aliasName.trim() },
    });
    setSaving(false);
    if (!r.ok) {
      toast.error(r.error);
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>Novo alias para {zone.display_name}</DialogTitle>
        </DialogHeader>
        <div>
          ...
          <Input
            ...
            placeholder="Ex.: Itaipuacu"
          />
          <p className="text-xs text-muted-foreground mt-1">
            Acentos e caixa serão normalizados automaticamente.
          </p>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>
