...
import { getFinanceMargin, updateProductCost, type FinanceMarginRow } from '@/server/finance.functions';

export const Route = createFileRoute('/admin/financeiro/margem')({
  head: () => buildSeo({ title: 'Margem de lucro', url: '/admin/financeiro/margem', noindex: true }),
    ...
    <AdminLayout title="Margem de lucro">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          {filters.map((f) => (
            <Button key={f.id} size="sm" variant={status === f.id ? 'default' : 'outline'} onClick={() => setStatus(f.id)}>
              {f.label}
            </Button>
          ))}
                  ...
                  return (
                    <tr key={r.id} className="border-t border-border">
                      <td className="p-3">
                        ...
                        <div className="text-xs text-muted-foreground">{r.sku ?? '—'} · {r.category_name ?? 'sem categoria'}</div>
                      </td>
                      ...
                      <td className="p-3 text-right">
                        {r.b2b_price != null ? (
                          <div>
                            ...
                            {r.b2b_status !== 'na' && <div className="mt-0.5">{statusBadge(r.b2b_status as FinanceMarginRow['status'])}</div>}
                          </div>
                        ) : '—'}
                      </td>
                      <td className="p-3 text-right">
                        {isEditing ? (
                          <Input className="h-8 w-24 text-right" type="number" step="0.01" value={editCost} onChange={(e) => setEditCost(e.target.value)} />
                        ) : brl(r.cost_price)}
                      </td>
                      <td className="p-3 text-right">
                        {r.margin_amount != null ? (
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">{r.margin_percent?.toFixed(1)}%</div>
                          </div>
                        ) : '—'}
                      </td>
                      <td className="p-3 text-right">
                        {isEditing ? (
                          <Input className="h-8 w-20 text-right" type="number" step="0.1" placeholder={String(r.effective_min_margin_percent)} value={editMin} onChange={(e) => setEditMin(e.target.value)} />
                        ) : (
                          <span title={r.min_margin_percent == null ? 'Usando padrão da loja' : 'Definida no produto'}>
                            {r.min_margin_percent != null ? `${r.min_margin_percent}%` : `${r.effective_min_margin_percent}% (padrão)`}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-3 text-right whitespace-nowrap">
                        {isEditing ? (
                          <div className="flex gap-1 justify-end">
                            ...
                            <Button size="sm" onClick={() => saveEdit(r.id)}><Save className="w-3 h-3 mr-1" />Salvar</Button>
                          </div>
                        ) : (
                          <Button size="sm" variant="outline" onClick={() => startEdit(r)}>Editar custo</Button>
                        )}
                      </td>
                    </tr>
                  );
