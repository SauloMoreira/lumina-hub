    ...
    >
      <div className="space-y-6">
        <p className="text-sm text-muted-foreground max-w-3xl -mt-2">
          ...
          <strong> Esta tela não altera nada automaticamente</strong> — apenas sinaliza o que merece revisão.
          {summary && (
            <>
              {' '}Análise baseada na janela configurada em <Link to={'/admin/produtos/estoque' as any} className="underline">Estoque</Link>:{' '}
              vendas dos últimos <strong>{summary.salesWindowDays}</strong> dias, produto parado quando sem venda há{' '}
              <strong>{summary.inactiveDaysThreshold}</strong> dias, alto giro a partir de{' '}
              <strong>{summary.highMovementMinQty}</strong> unidades vendidas no período.
            </>
          )}
        </p>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            <SummaryCard icon={DollarSign} label="Sem custo" value={summary?.noCost} loading={isLoading} tone="warn"
              onClick={() => { setFilter('no_cost'); setPage(1); }} active={filter === 'no_cost'} />
            <SummaryCard icon={Tag} label="Sem preço" value={summary?.noPrice} loading={isLoading} tone="danger"
              onClick={() => { setFilter('no_price'); setPage(1); }} active={filter === 'no_price'} />
            <SummaryCard icon={AlertOctagon} label="Margem negativa" value={summary?.negativeMargin} loading={isLoading} tone="danger"
              onClick={() => { setFilter('negative_margin'); setPage(1); }} active={filter === 'negative_margin'} />
            <SummaryCard icon={AlertTriangle} label="Margem crítica" value={summary?.criticalMargin} loading={isLoading} tone="danger"
              onClick={() => { setFilter('critical_margin'); setPage(1); }} active={filter === 'critical_margin'} />
            <SummaryCard icon={Info} label="Margem em atenção" value={summary?.attentionMargin} loading={isLoading} tone="warn"
              onClick={() => { setFilter('attention_margin'); setPage(1); }} active={filter === 'attention_margin'} />
            <SummaryCard icon={Briefcase} label="B2B crítico" value={summary?.b2bCritical} loading={isLoading} tone="danger"
              onClick={() => { setFilter('b2b_critical'); setPage(1); }} active={filter === 'b2b_critical'} />
            <SummaryCard icon={Briefcase} label="B2B incompleto" value={summary?.b2bIncomplete} loading={isLoading} tone="warn"
              onClick={() => { setFilter('b2b_incomplete'); setPage(1); }} active={filter === 'b2b_incomplete'} />
            <SummaryCard icon={CheckCircle2} label="Saudáveis" value={summary?.healthy} loading={isLoading} tone="ok"
              onClick={() => { setFilter('healthy'); setPage(1); }} active={filter === 'healthy'} />
          </div>

          <h2 className="font-display font-semibold text-base mt-5 mb-3">Resumo por giro (últimos {summary?.salesWindowDays ?? 30} dias)</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            <SummaryCard icon={TrendingDown} label="Parados com estoque" value={summary?.stalledWithStock} loading={isLoading} tone="warn"
              onClick={() => { setFilter('stalled'); setPage(1); }} active={filter === 'stalled'} />
            <SummaryCard icon={Flame} label="Alto giro" value={summary?.highMovement} loading={isLoading} tone="ok"
              onClick={() => { setFilter('high_movement'); setPage(1); }} active={filter === 'high_movement'} />
            <SummaryCard icon={AlertOctagon} label="Alto giro c/ margem baixa" value={summary?.highMovementLowMargin} loading={isLoading} tone="danger"
              onClick={() => { setFilter('high_movement_low_margin'); setPage(1); }} active={filter === 'high_movement_low_margin'} />
            <SummaryCard icon={Clock} label="Sem venda no período" value={summary?.noSalesInWindow} loading={isLoading} tone="info"
              onClick={() => { setFilter('no_sales_window'); setPage(1); }} active={filter === 'no_sales_window'} />
            <SummaryCard icon={DollarSign} label="Receita no período" value={undefined} valueOverride={summary ? fmtBRL(summary.revenueInWindow) : '—'} loading={isLoading} tone="ok" />
          </div>
          {summary && (
        ...
        <section className="flex flex-col lg:flex-row gap-2 lg:items-end">
          <form onSubmit={handleSearchSubmit} className="flex-1 min-w-0">
            <div className="relative">
              ...
              <Input placeholder="Buscar por nome ou SKU…" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="pl-8" />
            </div>
          </form>
          <Select value={filter} onValueChange={(v) => { setFilter(v as CommercialFilter); setPage(1); }}>
          ...
          <Select value={categoryId} onValueChange={(v) => { setCategoryId(v); setPage(1); }}>
            <SelectTrigger className="w-full lg:w-[200px]"><SelectValue placeholder="Categoria" /></SelectTrigger>
            <SelectContent>
          ...
          {(filterOptions?.brands?.length ?? 0) > 0 && (
            <Select value={brand} onValueChange={(v) => { setBrand(v); setPage(1); }}>
              <SelectTrigger className="w-full lg:w-[180px]"><SelectValue placeholder="Marca" /></SelectTrigger>
              <SelectContent>
          ...
          <Card className="overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {isLoading ? (
                    <tr><td colSpan={13} className="px-3 py-8 text-center text-muted-foreground">Carregando…</td></tr>
                  ) : rows.length === 0 ? (
                    <tr><td colSpan={13} className="px-3 py-8 text-center text-muted-foreground">Nenhum produto encontrado para os filtros atuais.</td></tr>
                  ) : (
            ...
            {total > pageSize && (
              <div className="flex items-center justify-between p-3 border-t border-border bg-muted/20">
                <p className="text-xs text-muted-foreground">Página {page} de {totalPages} • {total} produto(s)</p>
...
function SummaryCard({
  icon: Icon, label, value, valueOverride, loading, tone, onClick, active,
}: {
    ...
    <button
      ...
      className={`text-left rounded-xl border p-3 transition-colors ${
        active ? 'border-primary/60 bg-primary/5'
          : onClick ? 'border-border bg-card hover:bg-muted/50' : 'border-border bg-card cursor-default'
      }`}
    >
...
function ReviewRow({ row }: { row: CommercialReviewRow }) {
  const r = row.review;
  const marginCls =
    r.marginPercent == null ? 'text-muted-foreground'
      : r.marginPercent < 0 ? 'text-destructive font-medium'
      : r.marginPercent < r.effectiveMinMargin ? 'text-destructive'
      : r.marginPercent < r.effectiveMinMargin + 5 ? 'text-amber-600 dark:text-amber-400'
      : 'text-emerald-600 dark:text-emerald-400';
  return (
    <tr className={`border-t border-border hover:bg-muted/30 ${row.high_movement_low_margin ? 'bg-destructive/5' : ''}`}>
      ...
      <td className="px-3 py-2 align-top text-right tabular-nums">
        {row.sales.qty_sold_window > 0 ? row.sales.qty_sold_window : <span className="text-muted-foreground">0</span>}
        {row.sales.orders_count_window > 0 && (
          <div className="text-[10px] text-muted-foreground">{row.sales.orders_count_window} pedido(s)</div>
        )}
      </td>
      <td className="px-3 py-2 align-top text-right tabular-nums">
        {row.sales.revenue_window > 0 ? fmtBRL(row.sales.revenue_window) : <span className="text-muted-foreground">—</span>}
      </td>
      <td className="px-3 py-2 align-top text-xs">
        {fmtDate(row.sales.last_sold_at)}
        {row.sales.days_since_last_sale != null && (
          <div className="text-[10px] text-muted-foreground">há {row.sales.days_since_last_sale}d</div>
        )}
      </td>
      <td className="px-3 py-2 align-top">
      ...
      <td className="px-3 py-2 align-top">
        ...
        {row.high_movement_low_margin && (
          <div className="text-[10px] text-destructive mt-1 font-medium">Vende muito, margem baixa</div>
        )}
      </td>
      <td className="px-3 py-2 align-top">
        {r.issues.length === 0 ? (
              ...
              <li key={i.code} className="flex gap-1.5">
                <Sparkles
                  className={`w-3 h-3 flex-shrink-0 mt-0.5 ${
                    i.severity === 'high' ? 'text-destructive'
                      : i.severity === 'medium' ? 'text-amber-500'
                      : 'text-muted-foreground'
                  }`}
                />
                <span><span className="font-medium">{i.message}</span></span>
              </li>
            ))}
            {r.issues.length > 3 && (
