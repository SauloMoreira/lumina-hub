...
const EMPTY: Form = {
  name: '', description: '', trigger_type: 'cart_abandoned', channel: 'whatsapp',
  wait_minutes: 60, template_id: null, active: false, max_sends_per_entity: 1, respect_consent: true,
};
...
function AutomacoesPage() {
  ...
  const openEdit = (r: Rule) => { setForm({ ...r }); setOpen(true); };
  ...
  const runTest = async (r: Rule) => {
    ...
    const { error } = await supabase.from('automation_runs').insert({
      ...
      channel: r.channel,
      status: 'preview',
      generated_message: 'Teste manual — disparo automático real será habilitado após configurar provedor.',
    ...
    <AdminLayout
      ...
      action={
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={openNew}><Plus className="w-4 h-4 mr-1" /> Nova automação</Button>
        </div>
      }
    >
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4 text-sm mb-4">
        ...
        <p className="text-muted-foreground">
          Esta área já está estruturada para automações futuras. Por enquanto, todas as automações nascem
          inativas e só executam quando você usar “Testar manualmente”. O envio automático real será habilitado
          quando integrarmos um provedor oficial de WhatsApp/e-mail. {inactiveCount > 0 && (
            <span className="block mt-1">{inactiveCount} automação(ões) inativa(s) prontas para uso.</span>
          )}
        </p>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && rules.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">Nenhuma automação cadastrada.</td></tr>
              )}
              {!loading && rules.map((r) => (
                <tr key={r.id} className="border-t border-border hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3 text-xs">
                    <Badge variant="secondary">{TRIGGER_LABELS[r.trigger_type] ?? r.trigger_type}</Badge>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Button variant="ghost" size="icon" className="h-8 w-8" title="Testar manualmente" onClick={() => runTest(r)}>
                      <Play className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEdit(r)}>
                      <Pencil className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => del(r.id)}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Toda automação nasce inativa. Ative quando estiver tudo configurado.</DialogDescription>
          </DialogHeader>
          <form onSubmit={save} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setForm({ ...form, max_sends_per_entity: Number(e.target.value) || 1 })}
                />
              </div>
            </div>
            <div>
            ...
            <div className="flex items-center gap-4 pt-1">
              ...
              <div className="flex items-center gap-2">
                <Switch checked={form.respect_consent} onCheckedChange={(c) => setForm({ ...form, respect_consent: c })} />
      ...
      <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="max-h-[60vh] overflow-y-auto space-y-2">
            {history.length === 0 && <p className="text-sm text-muted-foreground py-6 text-center">Nenhum registro ainda.</p>}
            {history.map((h) => (
              <div key={h.id} className="border border-border rounded-md p-3 text-xs">
                <div className="flex items-center justify-between gap-2">
                  <Badge variant={h.status === 'sent' ? 'default' : h.status === 'error' ? 'destructive' : 'secondary'}>{h.status}</Badge>
                  <span className="text-muted-foreground">{new Date(h.created_at).toLocaleString('pt-BR')}</span>
                </div>
                <div className="mt-1 text-muted-foreground">{h.trigger_kind} · {h.channel} · {h.entity_type}</div>
                {h.generated_message && <div className="mt-1 whitespace-pre-wrap">{h.generated_message}</div>}
