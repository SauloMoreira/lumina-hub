-- Estende search_products_public para suportar a vitrine /atacado
-- Mantém compatibilidade total com a chamada atual da Onda 9B (todos os novos params são opcionais).

-- Drop antigo para poder mudar a coluna do RETURNS TABLE
DROP FUNCTION IF EXISTS public.search_products_public(text[], uuid, text, numeric, numeric, boolean, boolean, boolean, text, integer, integer);
...
BEGIN
  ...
  ORDER BY
    ...
    -- Maior desconto B2B (em %), considerando sale_price ou price como referência.
    -- Produtos sem b2b ficam com 0 e caem para o final.
